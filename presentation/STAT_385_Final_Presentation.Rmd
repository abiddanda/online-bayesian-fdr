...
<aside class="notes">
<small>
So in the online testing context we imagine that data is revealed to us sequentially through time. In this seeting we test hypothesis each each time point and control FDR at a given level without obersving all the data we could potentially see. Online testing is common in a wide range of scenarios including high frequency stock trading, A/B testing, and new genetic / clincal that include streaming data. 
</small>
</aside>
...
<aside class="notes">
<small>
In alpha-investing we define a wealth function w. When we reject the null the wealth function grows and when we fail to reject the null the wealth function decays. Specifically at time 0 we set the wealth function to the global level alpha. For step 2 we then proceed to set a time specific alpha sub t. We then reject or fail to reject the P value from time t and redefine our wealth function depeding on what desicion we make. The general features of alpha-investing are 

* the more discoveries we make the less stringent we are
* the less discoveries we make the more stingent we are 
through time. 
* signals have to be very strong if one has failed to rejct a lot.
</small>
</aside>
...
<aside class="notes">
* Some of the challenges in the previously outlined online testing appraoches include that strict desicions rules have to be made at each time point and errors in previous desicions propagate forward in time. 
* It is challenging to control both power and fdr when the time series data can be quite dynamic
* Heuristics don't allow directly modeling uncertanity in the streaming data 
</aside>
...
<aside class="notes">
<small>
As we covered in class we take a Bayesian appraoch to FDR control in a general setting (not necessirly related to online testing). In this approach we model a give test statistic such as a Z score coming from a mixture model with a null and signal component. As seen in the figure where the orange is the null component and the blueish color is the signal distribution.
</small>
</aside>
...
<aside class="notes">
<small>
More specifically let X be a test statistic, pi0 be the true proportion of nulls in our data, mu_1 is the mean of the signal component and sigma^2_1 be the variance of the signal component. One common assumption is to model X as a mixture of gaussian with the defined parameters.
</small>
</aside>
...
<aside class="notes">
<small>
Here we are assuming we know the parameters of the null component and have to estimate pi0, mu1, and sigma2_1. As we saw in our home work we can control for FDR at level alpha by finding x_hat and reject each test statistic if X > x_hat. One key problem we must ponder over (as seen in the pondering emoji) is how to esimate the unknown parameters.
</small>
</aside>
